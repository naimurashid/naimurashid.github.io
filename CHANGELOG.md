...
### Changed
- **Site `max_width` 1120px → 1080px → 1024px → 992px → 960px**. Iterative site-wide tightening to reduce visual sprawl on wide monitors. Empirical 3-col → 2-col breakpoint on the most-constrained grid (`.project-grid` with minmax(320px, 1fr)) is at 858px, so 960 retains a 102px safety buffer. 3-col card grids on `/software/`, `/projects/`, `/funding/` continue to render with cards at ~290–294px each. (Commits `8bf913dd`, plus follow-up tightening.)
