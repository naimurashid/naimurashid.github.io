...
### Changed
- **Site `max_width` 1120px → 1080px → 1024px → 992px**. Iterative site-wide tightening to reduce visual sprawl on wide monitors. 992 is Bootstrap's `lg` breakpoint and the iPad-portrait viewport. 3-column card grids on `/software/`, `/projects/`, `/funding/` compress slightly: each card lands at ~315px (instead of 320px), a sub-2% change. (Commits `8bf913dd`, plus follow-up tightening.)
