...
- **Heading register extended to `.page-section` wrappers** — "R Packages", "For Prospective PhD Students", "Publications by Theme" etc. were still rendering in the theme-default bold sans instead of the serif register, the same structural gap as the `.clearfix` fix
- **Prose em-dashes removed** on teaching, research, and Naim's member bio; funding SPORE card titles switched to colon form (list-separator em-dashes in funding/software/teaching lists left as an intentional format)
